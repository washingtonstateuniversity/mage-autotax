...
class Varien_Data_Collection_Db extends Varien_Data_Collection
{
    ...
    public function getSelectCountSql()
    {
        ...
        if (count($this->getSelect()->getPart(Zend_Db_Select::GROUP)) > 0) {
            ...
            $countSelect->distinct(true); // @codingStandardsIgnoreLine
            $group = $this->getSelect()->getPart(Zend_Db_Select::GROUP);
            $countSelect->columns("COUNT(DISTINCT ".implode(", ", $group).")"); // @codingStandardsIgnoreLine
        } else {
     ...
     * @return Varien_Data_Collection_Db
     */
    private function _setOrder($field, $direction, $unshift = false) // @codingStandardsIgnoreLine
    {
        $this->_isOrdersRendered = false;
    ...
    {
        $this->_select->distinct($flag); // @codingStandardsIgnoreLine
        return $this;
    }
    ...
    {
        if ($printQuery) {
            echo is_null($sql) ? $this->getSelect()->__toString() : $sql; // @codingStandardsIgnoreLine
        }
    ...
    protected function _fetchAll($select)
    {
        if ($this->_canUseCache()) {
            ...
            if ($data) {
                ...
                $data = $this->getConnection()->fetchAll($select, $this->_bindParams); // @codingStandardsIgnoreLine
